...
const { getSafesByOwner, getSafeInfo, getSafeService } = require("../gnosis");
const { getEthersAdapter, getProvider } = require("../wallet");
const networks = require("../networks");

exports.onUserWalletChange = onDocumentWritten("/users/{uid}/teams/{teamId}", async (event) => {
    ...
    if (utils.isAddress(walletAddress)) {
        try {
            ...
            for (const key in networks) {
                ...
                if (safeReturn?.safes.length > 0) {
                    await Promise.all(
                        safeReturn.safes.map(async (safe) => {
            ...
            const safesRef = db.collection("users").doc(event.params.uid).collection("teams").doc(event.params.teamId);
